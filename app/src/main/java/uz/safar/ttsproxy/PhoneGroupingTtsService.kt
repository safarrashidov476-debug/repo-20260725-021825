...
class PhoneGroupingTtsService : TextToSpeechService() {
    ...
    /**
     * Uzbek Latin -> Cyrillic single-letter map, used only for keyboard-letter
     * announcements. RHVoice apparently pronounces a bare Latin ASCII letter the
     * same way regardless of the active voice/locale (it's treated as a generic
     * symbol name, not language-specific text) - but genuine Cyrillic text is
     * confirmed to trigger the real Russian voice correctly. So instead of
     * sending "a" to the Russian voice and hoping it sounds different, we send
     * its Cyrillic counterpart "а", which the Russian voice reliably renders.
     */
    private val LATIN_TO_CYRILLIC_LETTER = mapOf(
        'a' to 'а', 'b' to 'б', 'd' to 'д', 'e' to 'е', 'f' to 'ф',
        'g' to 'г', 'h' to 'ҳ', 'i' to 'и', 'j' to 'ж', 'k' to 'к',
        'l' to 'л', 'm' to 'м', 'n' to 'н', 'o' to 'о', 'p' to 'п',
        'q' to 'қ', 'r' to 'р', 's' to 'с', 't' to 'т', 'u' to 'у',
    ...
    override fun onSynthesizeText(request: SynthesisRequest, callback: SynthesisCallback) {
        ...
        Log.d(TAG, "onSynthesizeText raw=${rawText.toCodePointDebugString()} singleLetter=${isSingleLetterAnnouncement(rawText)}")
        ...
        val baseLocale = if (isKeyboardLetter) {
